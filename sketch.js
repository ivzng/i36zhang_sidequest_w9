...
function keyPressed() {
  // Start music on any key press if it hasn't started yet
  startMusicIfNeeded();

  // F1 to toggle the overlay
  if (keyCode === 112) {
    // 112 is the specific keyCode for F1
    showDebug = !showDebug;
  }

  // 'm' to toggle moon gravity
  if (key === "m" || key === "M") {
    moonGravity = !moonGravity;
    // Apply the change immediately to the physics world
...
function draw() {
  ...
  camera.on();
  // Update the FPS display only every 10 frames
  if (frameCount % 10 === 0) {
    displayFPS = floor(frameRate());
  }
  // --- PLAYER CONTROLS ---
  ...
  if (!attacking) {
    ...
    // --- DEBUG OVERLAY (Draw last so it's on top) ---
    if (showDebug) {
      drawDebugScreen();
    }
  }

  // ... (keep all your existing player, level, and preload code) ...
  ...
  function draw() {
    ...
    // ... (keep all your existing movement and state machine code) ...
  }

  // Update keyPressed to handle the toggles
  function keyPressed() {
    startMusicIfNeeded();
    ...
    // Toggle Moon Gravity (only if debug is open)
    if (showDebug && (key === "m" || key === "M")) {
      moonGravity = !moonGravity;
    }
  }
  ...
  function drawDebugScreen() {
    ...
    text("DEBUG MENU (F1 to Close)", 15, 15);
    text(`Gravity: ${world.gravity.y.toFixed(2)}`, 15, 30);
    text(`Moon Gravity [M]: ${moonGravity ? "ON" : "OFF"}`, 15, 45);
    text(`Player Pos: ${floor(player.x)}, ${floor(player.y)}`, 15, 60);
    pop();
  }
  function drawDebugScreen() {
    push(); // Start a new drawing state
    camera.off(); // Ensure UI doesn't move with the player
    ...
    text("Gravity: " + world.gravity.y.toFixed(1), 10, 25);
    text("Moon Mode [M]: " + (moonGravity ? "ON" : "OFF"), 10, 40);
    text("FPS: " + displayFPS, 10, 55);
    camera.on(); // Turn camera back on for the next frame
